require "../../database_preparation/db_helper_bt_poly"
    ...
    describe "Plain" do
      ...
      describe "#relation" do
        ...
        it "should query dependent and return dependent with correct type" do
          db_helper.prepare
          db_helper.get_post_node_related_to_post_image.node.not_nil!.should be_a(
            PostImage
          )

          db_helper.get_post_node_related_to_post_text.node.not_nil!.should be_a(
            PostText
          )
        end
      ...
      describe "#relation(load: false)" do
        ...
        it "should not load related model" do
          db_helper.prepare
          db_helper.get_post_node_related_to_post_image.node(load: false).should be_nil
        end
      ...
      describe "#realation(yield_repository: true, &block)" do
        ...
        it "returns repository#where : QueryBuilder of related model for corresponding type" do
          db_helper.prepare
          db_helper.get_post_node_related_to_post_image.node(yield_repository: true) do |repo|
            repo.should be_a(Shepherd::Model::QueryBuilder::Adapters::Postgres::Where(Shepherd::Database::DefaultConnection, PostImage))
          end

          db_helper.get_post_node_related_to_post_text.node(yield_repository: true) do |repo|
        ...
        it "when #execute called on repository returned value should be assigned to #relation" do
          ...
          db_helper.get_post_node_related_to_post_image.node(load: false).not_nil!.should be_a(PostImage)

          db_helper.get_post_node_related_to_post_text.node(yield_repository: true) do |repo|
            repo.not_nil!.execute[0]?
          end
          db_helper.get_post_node_related_to_post_text.node(load: false).not_nil!.should be_a(PostText)
        end
