...
# this class sole responsibility: if file exists and path is GET and not /, send it
# And should be used only for development, in other cases you will anyway use proxy server as NGINX for example
...
class Shepherd::Server::Handlers::StaticFile < HTTP::Handler
  ...
  def call(context : HTTP::Server::Request) : Nil
    
    #checks for correct method and that path is not /
    unless context.request.method == "GET" || context.request.method == "HEAD" || context.path == "/"
      call_next(context)
    end
